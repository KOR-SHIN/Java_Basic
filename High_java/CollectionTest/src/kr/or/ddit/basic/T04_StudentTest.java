	...
	public static void main(String[] args) {
		...
		for(int i=0; i<list.size(); i++) {
			...
			for(int k=0; k<list.size(); k++) {
				if(list.get(i).getTotal_score() < list.get(k).getTotal_score()) {
					temp++;
				}
			}
			list.get(i).setGrade(temp);
		}
...
class ScoreDesc implements Comparator<Student> {
	...
	public int compare(Student st1, Student st2) {
		...
		if(st1.getTotal_score() == st2.getTotal_score()) {
			// 총점이 같으면 학번기준 내림차순으로 정렬하도록 구현
			// Student 클래스에 구현해놓은 학번기준 오름차순 정렬 결과에 -1을 곱하면 내림차순이 된다.
			return st1.compareTo(st2) * -1;
		}
		
		// Wrapper 클래스에 오버라이드되어있는 compare 메서드를 사용
